...
class PostModel {
    ...
    public function getFeedPosts($userId) {
        $query = "SELECT photos.*, COUNT(photo_likes.like_id) AS like_count, COUNT(photo_comments.comment_id) AS comment_count
                  FROM photos
                  LEFT JOIN photo_likes ON photos.photo_id = photo_likes.photo_id
                  LEFT JOIN photo_comments ON photos.photo_id = photo_comments.photo_id
                  WHERE photos.user_id IN (SELECT user_id2 FROM friendships WHERE user_id1 = $userId AND status = 'accepted')
